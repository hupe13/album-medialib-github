...
function album_medialib_textdomain() {
	if ( file_exists( ALBUM_MEDIALIB_DIR . '/lang/album-media-library-' . get_locale() . '.mo' ) ) {
		$mofile = ALBUM_MEDIALIB_DIR . '/lang/album-media-library-' . get_locale() . '.mo';
		load_textdomain( 'album-media-library', $mofile );
	}
}
add_action( 'plugins_loaded', 'album_medialib_textdomain' );

// Updates from Github
function album_medialib_updates_from_github() {
